...
void main() {
  group('MonthView', () {
    ...
    testWidgets(
      ...
      // Jan, Feb, Mar are before minDate — find Jan (disabled)
      final disabledMonth = find.byWidgetPredicate((widget) {
        if (widget is ExcludeSemantics) {
          Widget? inner = widget.child;
          if (inner is Padding) inner = inner.child;
          if (inner is Container && inner.child is Center) {
            final text = (inner.child as Center).child;
            if (text is Text && text.data == 'Jan') return true;
          }
        }
        return false;
      });

      expect(disabledMonth, findsOneWidget);
      await tester.tap(disabledMonth, warnIfMissed: false);
      await tester.pump();
    ...
    testWidgets('uses theme cellsPadding for month cells',
        ...
        MaterialApp(
          home: Material(
            child: MonthView(
              ...
              maxDate: DateTime(2020, 12, 1),
              displayedDate: currentDate,
              theme: const MonthsPickerTheme(cellsPadding: customPadding),
            ),
          ),
        ),
      );
      ...
      final paddingFinder = find
          .descendant(
            ...
            matching: find.widgetWithText(Padding, 'Feb'),
          )
          .at(0);
